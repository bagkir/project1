...
def blog(request):
    return render(request, 'blog/index.html', context={'posts': random.choices(posts_dict, k=3)})


def posts(request, name_post):
    return render(request, 'blog/detail_by_name.html', context={'name_post': name_post})


def posts_by_number(request, number_post):
    return render(request, 'blog/detail_by_number.htm.html', context={'number_post': number_post})
